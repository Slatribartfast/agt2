...
def kick_random(input_list):
    input_list.pop(random.randrange(len(input_list)))
    return input_list


def kick_least_populated(input_list):
    c_pop_l = []
...
def get_loosing_coalition(loosing_coals_all=[], max_out=5, max_overall: int = 15, min_gain: int = 6,
                          kick_strategy: str = "random", kick_rate: int = 100, give_up_rate: int = 1000,
                          min_avg_pop_size=0.645) -> list[list]:
    ...
    while len(loosing_coals) < max_overall - 1:
        ...
        if c_bad_luck % kick_rate == 0:
            ...
            if kick_strategy == "random":
                ...
                loosing_coals = kick_least_populated(loosing_coals)
